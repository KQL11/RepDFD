...
class FFDataset(Dataset):
    def __init__(self, root_path="", video_names=[], phase='train', test_frame_nums=5, vid_sample=1, is_pillow=True,
        ...
        if phase != 'train':
            ...
            for name in image_name:
                ...
                #                         '/data2/linkaiqing/code/Reprogramming/dataset/CropFace_Embedding/Transface')
                new_name = name.replace('/data2/linkaiqing/code/Reprogramming/dataset/', '/data2/linkaiqing/code/Reprogramming/dataset/CropFace_Embedding/Transface/')
    ...
    def load_image_name(self):
        ...
        for video_name in tqdm(self.videos_by_class['real'] + self.videos_by_class['fake']):
            ...
            if self.test_frame_nums == -1:
                pass
            elif len(frame_names) > self.test_frame_nums:
    ...
    def __getitem__(self, index):
        if self.phase == 'train':
            ...
            video_path = os.path.join(self.root_path, video_name)
            video_path = video_path.replace('/data2/linkaiqing/code/Reprogramming/dataset',
                                              '/data2/linkaiqing/code/Reprogramming/dataset/CropFace_Embedding/Transface')
            image_names = os.listdir(video_path)
            image_names = [i.replace('.pt', '.png') for i in image_names]
            image_name = random.sample(image_names, 1)[0]
            video_path = video_path.replace('/data2/linkaiqing/code/Reprogramming/dataset/CropFace_Embedding/Transface',
                                            '/data2/linkaiqing/code/Reprogramming/dataset')
            ...
            if self.is_pillow:
                ...
                #     image = t(image)
                #
                #     image.save('/data2/linkaiqing/code/Reprogramming/dataset/code/Reprogramming/VP_Deepfake/x.png')
            ...
            face_emb = torch.load(((image_path.replace('/data2/linkaiqing/code/Reprogramming/dataset',
                                                       '/data2/linkaiqing/code/Reprogramming/dataset/CropFace_Embedding/Transface').replace(
...
class CDFDataset(Dataset):
    def __init__(self, root_path="", video_names=[], phase='train', test_frame_nums=10, vid_sample=1, is_pillow=True,
        ...
        self.num_classes = num_class
        self.transform = transform
        self.test_frame_nums = test_frame_nums      # If test_frame_nums = -1, load all frames
        self.videos_by_class = self.load_video_name()  # load all video name
        if phase != 'train':
            image_name = self.load_image_name()
            self.image_name = []
            for name in image_name:
                new_name = name.replace('/data2/linkaiqing/code/Reprogramming/dataset/Celeb_DF',
                                        '/data2/linkaiqing/code/Reprogramming/dataset/CropFace_Embedding/Transface/Celeb_DF')
                if os.path.exists(new_name.replace('.png', '.pt')):
                    self.image_name.append(name)
        else:
    ...
    def load_image_name(self):
        image_names = []
        for video_name in tqdm(self.videos_by_class['real'] + self.videos_by_class['fake']):
        # for video_name in tqdm(self.videos_by_class['fake']):
    ...
    def __getitem__(self, index):
        if self.phase == 'train':
            ...
            # Face Embed Load
            face_emb = torch.load(os.path.join(video_path.replace('/data2/linkaiqing/code/Reprogramming/dataset/Celeb_DF',
                                                                  '/data2/linkaiqing/code/Reprogramming/dataset/CropFace_Embedding/Transface/Celeb_DF'),
                                               image_name.replace('.png', '.pt')), map_location='cpu')
            return image, label, face_emb
            ...
            # Face Embed Load
            face_emb = torch.load(((image_path.replace('/data2/linkaiqing/code/Reprogramming/dataset/Celeb_DF',
                                                       '/data2/linkaiqing/code/Reprogramming/dataset/CropFace_Embedding/Transface/Celeb_DF').replace(
                '.png', '.pt'))), map_location='cpu')
            return image, label, face_emb
