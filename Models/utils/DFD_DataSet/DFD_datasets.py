...
class FFDataset(Dataset):
    def __init__(self, root_path="", video_names=[], phase='train', test_frame_nums=5, vid_sample=1, is_pillow=True,
                 num_class=2, transform=None, face_model='Transface', aug=False):
        ...
        if phase != 'train':
            ...
            for name in image_name:
                new_name = name.replace(f'/data2/linkaiqing/code/Reprogramming/dataset/', 
                                        f'/data2/linkaiqing/code/Reprogramming/dataset/Face_Embedding/{self.face_model}/')
    ...
    def __getitem__(self, index):
        if self.phase == 'train':
            ...
            face_emb = torch.load(os.path.join(video_path.replace('/data2/linkaiqing/code/Reprogramming/dataset',
                                                                  f'/data2/linkaiqing/code/Reprogramming/dataset/Face_Embedding/{self.face_model}'),
                                               image_name.replace('.png', '.pt')), map_location='cpu').detach()
            # print(f"{image_name.replace('.png', '.pt')}")
            ...
            face_emb = torch.load(((image_path.replace('/data2/linkaiqing/code/Reprogramming/dataset',
                                                       f'/data2/linkaiqing/code/Reprogramming/dataset/Face_Embedding/{self.face_model}').replace(
                '.png', '.pt'))), map_location='cpu').detach()
            return image, label, face_emb
...
class CDFDataset(Dataset):
    def __init__(self, root_path="", video_names=[], phase='train', test_frame_nums=10, vid_sample=1, is_pillow=True,
                 num_class=2, face_model='Transface', transform=None):
        assert phase in ['train', 'valid', 'test']
        self.is_pillow = is_pillow
        self.vid_sample = vid_sample
        self.root_path = root_path
        ...
        if phase != 'train':
            ...
            for name in image_name:
                new_name = name.replace('/data2/linkaiqing/code/Reprogramming/dataset/Celeb_DF',
                                        f'/data2/linkaiqing/code/Reprogramming/dataset/Face_Embedding/{self.face_model}/Celeb_DF')
                if os.path.exists(new_name.replace('.png', '.pt')):
                    self.image_name.append(name)
        else:
    ...
    def __getitem__(self, index):
        if self.phase == 'train':
            ...
            face_emb = torch.load(((image_path.replace('/data2/linkaiqing/code/Reprogramming/dataset/Celeb_DF',
                                                       f'/data2/linkaiqing/code/Reprogramming/dataset/Face_Embedding/{self.face_model}/Celeb_DF').replace(
                '.png', '.pt'))), map_location='cpu').detach()
            return image, label, face_emb
